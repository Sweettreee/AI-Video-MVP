...
> 나중에 팀원이 image.py, video.py, merge.py를 여기에 추가하면 됨.
...
| scene.py | confirm 후 JSON 변환 결과의 스키마 (main_character, action, pose 등 11개 필드) | plain text → JSON 변환 후 필수 필드가 다 있는지 자동 검증. 빠진 필드가 있으면 Nano Banana 2에 넘기기 전에 잡아냄. |
...
| plot_evaluator.py | Code Grader(규칙 기반) + Model Grader(Structured Outputs + 확장 사고). C5 unsafe 사전 검수는 Haiku로 저비용 처리. Model Grader API 실패 시 Code fallback. | 자동 평가 로직. Structured Outputs로 JSON 파싱 실패 원천 제거. 확장 사고로 채점 품질 향상. |
| plot_advisor.py | 평가 결과를 기반으로 유저에게 보여줄 가이드라인 리포트 생성. previous_advice로 중복 안내 방지. API 실패 시 정적 가이드 fallback + 무한 루프 방지 (3회 템플릿, 5회 부분 진행) | Claude는 코치이지 작가가 아님. 유저가 직접 수정할 수 있도록 가이드. |
| plot_converter.py | confirm 후 plain text → JSON 파싱. 파싱 에러 시 구체적 위치 안내, 타입 불일치 시 캐스팅+경고, 파일 저장 실패 시 터미널 출력 fallback | Nano Banana 2에 넘길 수 있는 구조화된 데이터를 만듦. |
...
| 파일 | 역할 | 왜 필요한가 |
|------|------|------------|
| prompt.py | compose_image_prompt() — Nano Banana 2용 프롬프트 조합 | confirm 후 JSON 데이터를 Nano Banana 2 템플릿 문자열로 조합하는 함수. 지금 당장은 안 쓰지만, 팀원이 image 쪽 작업할 때 바로 가져다 쓸 수 있도록 자리만 잡아둔 것. |
...
services/plot_evaluator.py         ← prompts/eval.py
    ...
    │
    ├─ 5점 이상 → 유저에게 plain text 제시 → Human Eval 진입
    │
    └─ 5점 미만 → services/plot_advisor.py  ← prompts/advisor.py
                  ...
                  [유저에게 가이드라인 리포트 제공]
                      │  "현재 4.2점"
                      │  "Cut 3: 캐릭터 외형 묘사 없음"
                      │  "추천: 헤어스타일, 의상 색상을 추가해보세요"
                      │
                      ▼
                  [유저가 수정 포인트만 입력 → generate_plot(previous_storyboard + modification)]
                      │
                      ▼
                  plot_generator.py → 수정된 입력으로 재생성
                      │
                      ▼
                  plot_evaluator.py → 재평가
                      │
                      └─→ 5점 이상이 될 때까지 루프
    │
    ▼
